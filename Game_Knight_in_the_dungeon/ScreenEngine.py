...
class MiniMap(ScreenHandle):
    ...
    def draw_hero(self):
        self.game_engine.hero.draw_mini(self)

    def draw_map(self):
        size = self.game_engine.sprite_size//10
        # calculate (min_x,min_y) - left top corner
        min_x = 0
        min_y = 0
        min_x, min_y = self.calculate(min_x, min_y)
    ...
    def draw(self, canvas):
        ...
        ### multicolor
        for obj in self.game_engine.objects:
            if type(obj).__name__ == 'Ally':
                if obj.action.__name__ == 'reload_game':
                    pygame.draw.circle(
                        self, (221, 26, 235),
                        (obj.position[0]*6+3, obj.position[1]*6+3), 3)
                elif obj.action.__name__ == 'add_gold':
                    pygame.draw.circle(
                        self, (255, 228, 74),
                        (obj.position[0]*6+3, obj.position[1]*6+3), 3)
                ### for super mario
                elif obj.action.__name__ == 'apply_supermario':
                    pygame.draw.circle(
                        self, (0, 0, 255),
                        (obj.position[0]*6+3, obj.position[1]*6+3), 3)
                ####
...
class HelpWindow(ScreenHandle):
    ...
    def __init__(self, *args, **kwargs):
        ...
        ### for mini map
        self.data.append(["About mini Map:", ""])
        self.data.append(["White", "Hero"])
        self.data.append(["Golden", "Chest of gold"])
        self.data.append(["Green", "Ally"])
        self.data.append(["Red", "Enemy"])
        self.data.append(["Purple", "Stair"])
        self.data.append(["Blue", "Super Mario"])
        ####
